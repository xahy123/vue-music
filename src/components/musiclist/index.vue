    ...
    <div class="ww" ref="ww">
      ...
      <ul>
        <li v-for='(i,index) in songList'
          :key="i.key" @click="addPlayerList(i,index)">
          <div class="rank">
            {{index+1}}
          </div>
          <div class="r">
            <p>{{i.name}}</p>
            <p><span v-for="b in i.singer" :key="b.key">{{b.name}} . </span>{{i.album}}</p>
          </div>
        </li>
      </ul>
      </scroll>
  ...
  methods: {
    watchPlayerList(playerList){
         if(playerList.length>0){
          this.$refs.ww.style.bottom = "40px"
          this.$refs.ww.style.top = "270px"
          this.$refs.list.refresh()
        }else{
          this.$refs.ww.style.bottom = "0px"
          this.$refs.ww.style.top = "270px"
          this.$refs.list.refresh()
        }
    },
    ...
    //addPlayerList 是给歌绑定的点击事件
    addPlayerList(i,index){
      //list是获取列表 index是获取索引，都是从actions那边传过来的
      this.addPlayer({
        list:this.songList,
        index:index
      })
